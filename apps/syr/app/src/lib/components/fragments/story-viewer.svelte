<script lang="ts">
	...
	const name = $derived(profile?.displayName || profile?.username || 'Unknown');
	...
	>
		{#if slides.length > 0 && currentSlide}
			<div class="relative flex h-[100dvh] w-full items-center justify-center">
				<!-- Progress bars -->
				<div class="absolute top-0 right-0 left-0 z-20 flex gap-1 px-4 pt-3">
				...
				<!-- Header -->
				<div
					class="absolute top-0 right-0 left-0 z-10 flex items-center gap-3 bg-gradient-to-b from-black/70 to-transparent px-4 pt-6 pb-4"
				>
					<Avatar.Root class="h-9 w-9">
						{#if profile?.avatarUrl}
							<Avatar.Image src={profile.avatarUrl} alt={name} />
						{/if}
					...
					<div class="min-w-0 flex-1">
						...
						{#if currentSlide.published_at}
							<p class="truncate font-mono text-[11px] text-white/60">
								{relativeTime(currentSlide.published_at)}
							</p>
						{/if}
					</div>
					<button
				...
				<button
					type="button"
					class="absolute top-16 bottom-0 left-0 w-1/3"
					onclick={prev}
					aria-label="Previous"
				></button>
				<button
					type="button"
					class="absolute top-16 right-0 bottom-0 w-2/3"
					onclick={advance}
					aria-label="Next"
				></button>
				...
				{#if index > 0}
					<button
						type="button"
						class="absolute top-1/2 left-2 z-10 -translate-y-1/2 rounded-full bg-white/10 p-2 text-white/80 hover:bg-white/20 hover:text-white"
						onclick={prev}
						aria-label="Previous"
					>
				...
				{#if index < slides.length - 1}
					<button
						type="button"
						class="absolute top-1/2 right-2 z-10 -translate-y-1/2 rounded-full bg-white/10 p-2 text-white/80 hover:bg-white/20 hover:text-white"
						onclick={advance}
						aria-label="Next"
					>

<script lang="ts">
	...
	let step = $state<'warning' | 'syner'>('warning');
	...
	<Dialog.Content class="max-w-md">
			...
			<Dialog.Description>
				{#if step === 'warning'}
					<p class="text-sm text-muted-foreground">
						Remove the server-stored encrypted key (Aegis) from your identity. You must have
						<strong>exported your Sigil or Persona</strong> and imported it into Syner first.
					</p>
				...
				{:else}
					<p class="text-sm text-muted-foreground">
						...
						must have imported your persona (from exported Sigil/Persona) into Syner — this proves
		...
		<div class="space-y-4 py-4">
			{#if step === 'warning'}
				...
				<p class="text-sm text-muted-foreground">
					Sign with Syner to prove you have backed up your keys:
				</p>
		...
		<Dialog.Footer>
			{#if step === 'warning'}
				...
				<Button variant="destructive" onclick={handleVerifyWithSyner} disabled={deleting}>
					{#if deleting}

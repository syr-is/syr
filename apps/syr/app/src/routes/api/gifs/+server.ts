...
export const GET: RequestHandler = async ({ url, locals }) => {
	...
	const { data, total } = await gifController.getUserGifs(user.id, { limit, offset });
...
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	try {
		...
		const result = await gifController.createGif(user, data, { localId: gif_local_id });
		...
		throw error(500, { code: 'INTERNAL_SERVER_ERROR', message: 'Failed to create GIF' });
	}
};

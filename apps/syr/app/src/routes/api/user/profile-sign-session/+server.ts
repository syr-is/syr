...
import { ProfileSignedPayloadV1Schema } from '@syr-is/types';
...
/**
 * POST /api/user/profile-sign-session
 * Start Syner-assisted signing for a profile@v1 payload (same KV + post-sign routes as posts).
 */
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (!did || !did.startsWith('did:syr:')) {
		throw error(400, {
			code: 'BAD_REQUEST',
			message: 'Your account needs a DID before signing profile updates with Syner.'
		});
	}
	...
	const parsed = ProfileSignedPayloadV1Schema.safeParse(rawPayload);
	if (!parsed.success) {
		throw error(400, {
			code: 'VALIDATION_ERROR',
			message: 'Invalid profile@v1 payload',

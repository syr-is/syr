...
/**
 * POST /api/user/comment-sign-session
 * Start Syner-assisted signing for a comment@v1 payload.
 * Mirrors post-sign-session but accepts comment payloads.
 */
export const POST: RequestHandler = async ({ request, locals }) => {
	...
	if (!did || !did.startsWith('did:syr:')) {
		throw error(400, {
			code: 'BAD_REQUEST',
			message: 'Your account needs a DID before signing comments with Syner.'
		});
	}
	...
	if (!rootPk) {
		throw error(400, { code: 'NO_IDENTITY', message: 'No identity public key on record.' });
	}
	...
	// Validate comment@v1 payload shape
	if (
		rawPayload == null ||
		typeof rawPayload !== 'object' ||
		(rawPayload as Record<string, unknown>).type !== 'comment@v1'
	) {
		throw error(400, {
			code: 'VALIDATION_ERROR',
			message: 'Invalid comment@v1 payload: missing or wrong type field'
		});
	}

	const payloadObj = rawPayload as Record<string, unknown>;
	if (payloadObj.did !== did) {
		throw error(400, {
	...
	const payload = structuredClone(payloadObj);

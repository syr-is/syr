...
import { subscribeDeleteAegis } from '$lib/server/export-verify-broadcast';
import { getDeleteAegisChallenge } from '$lib/server/export-verify-store';
...
/**
 * GET /api/identity/delete-aegis-heartbeat?challenge_id=xxx
 *
 * SSE stream — waits for delete-aegis verification. Sends "delete_aegis_verified" with delete_aegis_token when Syner signs.
...
export const GET: RequestHandler = async ({ request, url, getClientAddress, locals }) => {
	...
	const challenge = await getDeleteAegisChallenge(challengeId);
	if (!challenge) {
	...
	const stream = new ReadableStream({
		start(c) {
			...
			unsubscribe = subscribeDeleteAegis(challengeId, (deleteAegisToken) => {
				send('delete_aegis_verified', JSON.stringify({ delete_aegis_token: deleteAegisToken }));
				cleanup();
			});

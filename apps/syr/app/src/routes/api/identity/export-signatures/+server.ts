...
export const POST: RequestHandler = async ({ request }) => {
	try {
		...
		// Verify each signature (use cached items when available to avoid O(n) rebuild per chunk)
		...
		if (!allReceived) {
			// Update session and return next chunk (use cached items when available)

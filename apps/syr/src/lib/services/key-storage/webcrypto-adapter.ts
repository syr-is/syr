...
async function getOrCreateKek(): Promise<CryptoKey> {
	...
	const key = await crypto.subtle.generateKey({ name: 'AES-GCM', length: KEY_LENGTH }, true, [
		'encrypt',
		'decrypt'
	]);
	const exported = await crypto.subtle.exportKey('raw', key);
	sessionStorage.setItem(KEK_STORAGE_KEY, btoa(String.fromCharCode(...new Uint8Array(exported))));
	return key;
}

async function encryptPrivateKey(kek: CryptoKey, pkcs8: ArrayBuffer): Promise<ArrayBuffer> {
	const iv = crypto.getRandomValues(new Uint8Array(IV_LENGTH));
	const cipher = await crypto.subtle.encrypt({ name: 'AES-GCM', iv }, kek, pkcs8);
...
async function decryptPrivateKey(kek: CryptoKey, combined: ArrayBuffer): Promise<ArrayBuffer> {
...
function createRecord(
	...
	const record: KeyRecord = {
		...
		type,
		createdAt,
		sign: (payload: string | Uint8Array) => signWithCryptoKey(payload, privateKey)
	};
...
export class WebCryptoKeyAdapter extends KeyStorageAdapter {
	async generateRootKeypair(): Promise<KeyRecord> {
		const { publicKey, privateKey } = await generateEd25519KeyPairWebCrypto();
		return createRecord('root', publicKey, 'root', new Date().toISOString(), privateKey);
	}

	async generateDeviceKeypair(): Promise<KeyRecord> {
		const { publicKey, privateKey } = await generateEd25519KeyPairWebCrypto();
		return createRecord('device', publicKey, 'device', new Date().toISOString(), privateKey);
	}
	...
	async getKey(id: string): Promise<KeyRecord | null> {
		...
		return createRecord(row.id, row.publicKey, row.type, row.createdAt, privateKey);
	}
	...
	async getAllKeys(): Promise<KeyRecord[]> {
		...
		for (const row of rows) {
			const pkcs8 = await decryptPrivateKey(kek, row.encryptedPrivateKey);
			const privateKey = await importPrivateKeyFromStorage(pkcs8);
			out.push(createRecord(row.id, row.publicKey, row.type, row.createdAt, privateKey));
		}
		return out;
	}

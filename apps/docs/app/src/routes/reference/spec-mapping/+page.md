...
| Signing UX preferences + Syner Sigil handoff       | **Partial**     | User prefs API; **Settings → Signing**: file load + **Syner QR/deeplink** handoff (`POST /api/user/sigil-handoff-session`, poll `GET …/sigil-handoff/[id]`, Syner `POST …/payload`); `sigil-session.ts` + WASM `signMutationPayload()` / `getUnlockedSigningSeed()`. Wired signing in forms still evolving.                     |

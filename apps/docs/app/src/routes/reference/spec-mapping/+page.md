...
| Cross-instance import of a rotated identity  | **Out of scope** | The export bundle does not carry the rotation chain, so an importing instance cannot resolve a rotated identity's current root; `validateBundle` rejects such bundles (`KEY_MISMATCH`). Rotated identities stay fully resolvable on their home instance. See [rotation spec §10.1](/architecture/recovery-rotation). |

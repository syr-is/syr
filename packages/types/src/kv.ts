...
export const KvIdStringSchema = z
	.string()
	.regex(KV_ID_REGEX, {
		message: "KV ID must be in format 'kv:type:index' (e.g., 'kv:session:abc123')"
	})
...
export const KvEntryCreateSchema = z.object({
	...
	index: z.string().regex(/^[a-zA-Z0-9_:-]+$/, {
		message: 'Index must contain only alphanumeric characters, underscores, hyphens, and colons'
	}),

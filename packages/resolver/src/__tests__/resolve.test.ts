...
function mockFetchSequence(...responses: Array<{ status: number; body?: unknown; ok?: boolean; throws?: boolean }>) {
  let callIndex = 0;
  return vi.fn(async () => {
...
describe("resolveDid", () => {
  ...
  it("throws NOT_FOUND for 404 registry response", async () => {
    ...
    await expect(
      resolveDid(did, { registryUrl: "https://registry.example.com" }),
  ...
  it("throws PROVIDER_UNREACHABLE when provider fetch fails", async () => {
    ...
    vi.stubGlobal(
      ...
      mockFetchSequence(
        { status: 200, body: record },
        { status: 200, body: record, throws: true },
      ),
    );
